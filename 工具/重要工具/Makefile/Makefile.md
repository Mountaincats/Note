...
*   **自动变量** (在规则的命令列表中使用)
    ...
    *   **`$?`**: 所有比目标新的依赖列表。
    *   **`$^`**: 规则中所有依赖的列表。
    *   **`$*`**: 当前规则中匹配的词干，即`%`，比如规则 `Makefile.%: tests/%.c` 匹配 `Makefile.foo` 时，`$*`的值就是 `foo`。
*   **常用的预定义变量**
    *   **`CC`**: C编译器的名字，缺省是`cc`。
    *   **`CFLAGS`**: C编译器的选项。
    *   **`CPPFLAGS`**: C预处理器的选项。
    *   **`RM`**: 删除命令，缺省是`rm -f`。
    *   **`MAKECMDGOALS`**: 传递用户原始调用目标，比如make run时，$(MAKECMDGOALS)是run
*   **变量**
    *   **`=`**
    *   **`:=`**
    *   **`?=`**
    *   **`+=`**
*   **`include` 指令**: 包含其他使用Make语法的文件。
    **`-include` 指令**: 包含其他使用Make语法的文件，当被包含的文件不存在时，不会报错。
    *   make会把include的文件名也当作目标来尝试更新，执行它们的命令列表，若无法更新则根据有无`-`来报错。
...
*   **`make` 常用命令行选项**
    ...
    *   **`-f <dir>`**: 指定Makefile文件，而不使用默认的 Makefile ，保持当前目录
    *   **`-p`**: 打印隐含规则数据库。
...
*   **Makefile 核心函数列表**
    ...
    | **信息输出** | `$(info text…)` | 输出信息 |
    | **变量相关** | `$(flavor VAR)` | 获取变量类型，返回undefined(未定义)或simple(定义) |
    | **make命令转换** | `$(eval 代码段)` | 将括号内的代码段作为 Makefile 代码解析 |
